...
//.logicking guidebot >>
void Player::setDamageState(DamageState state)
{
	Parent::setDamageState(state);
	if (mDamageState != Enabled && m_worldObject && !m_worldObject->getDestroyed())
	{
		m_worldObject->setDestroyed(true);
	}
}

void Player::createWorldObject()
{
	m_worldObject = (GuideBot::WorldObject*) new SceneWorldObject(this,GuideBot::WorldObject::WO_ACTOR);
	m_worldObject->registerObject();
}

void Player::destroyWorldObject()
{
...
//   setActionThread(PlayerData::RootAnim,true,false,false); // DS CRASH HERE
