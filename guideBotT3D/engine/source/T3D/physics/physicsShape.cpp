...
bool PhysicsShape::_createShape()
{
   ...
   mWorld = PHYSICSMGR->getWorld( isServerObject() ? "server" : "client" );
	...
	//.logicking guidebot>> create collision shape with object scale
   
	PhysicsCollision* colShape = db->colShape;
	if (!getScale().isUnitLength())
	{
		colShape = db->shape->buildColShape( false, getScale());
	}
	if (!colShape)
	{
		Con::errorf( "PhysicsShape::_createShape() - Can't find collision shape!" );
		return false;
	}
	//.logicking guidebot<<
